...
@allure.feature('14.11:Функциональное тестирование MPLS TE AutoBandwidth overflow')
@allure.title('Проверка перестроения RSVP LSP при изменении объема передаваемого через него трафика с включенным overflow')
@pytest.mark.part14_11
@pytest.mark.mpls_te_autobandwidth_overflow_test2
...
def test_mpls_te_autobandwidth_overflow_test2_part14_11 (ip, hostname, login, password, tun_name): 
    if hostname==DUT1['hostname']:
    	allure.attach.file('./network-schemes/part14_11_mpls_te_ab_atAR1_test2.png','Схема теста:', attachment_type=allure.attachment_type.PNG)
    ...
    if hostname==DUT2['hostname']:
    	allure.attach.file('./network-schemes/part14_11_mpls_te_ab_atAR2_test2.png','Схема теста:', attachment_type=allure.attachment_type.PNG)
...
# Перед началом теста необходимо  добавить в конфигурацию параметр  overflow-limit 3 
