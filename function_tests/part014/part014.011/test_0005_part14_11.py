...
@allure.feature('14.11:Функциональное тестирование MPLS TE AutoBandwidth underflow')
@allure.title('Проверка перестроения RSVP LSP при изменении объема передаваемого через него трафика с включенным underflow')
@pytest.mark.part14_11
@pytest.mark.mpls_te_autobandwidth_underflow_test3
...
def test_mpls_te_autobandwidth_over_under_flow_test3_part14_11 (ip, hostname, login, password, tun_name): 
    if hostname==DUT1['hostname']:
        allure.attach.file('./network-schemes/part14_11_mpls_te_ab_atAR1_test3.png','Схема теста:', attachment_type=allure.attachment_type.PNG)
    ...
    if hostname==DUT2['hostname']:
        allure.attach.file('./network-schemes/part14_11_mpls_te_ab_atAR2_test3.png','Схема теста:', attachment_type=allure.attachment_type.PNG)
...
# Определим ожидаемое время переключения RSVP LSP в данном тесте
    blue_red_switch_time=200 # Время генерации трафика в течение которого должно быть переключение с синего на красный путь, иначе тест будет считаться не успешным на шаге 2
    red_blue_switch_time=400 # Время в течение которого RSVP LSP (без трафика) должен вернуться с красного на синий путь, иначе тест будет считаться не успешным на шаге 3

# Перед началом теста необходимо  добавить в конфигурацию параметр  underflow-limit 3 
# тем самым попытка сигнализации RSVP LSP с новым измеренным bandwidth произойдет не через 300 секунд, а через 3*60=180 секунд
    conn.execute('configure') 
    conn.execute('mpls rsvp tunnel %s'%tun_name)
    conn.execute('auto-bandwidth overflow-limit 3')
    conn.execute('auto-bandwidth underflow-limit 3')
...
# Шаг2. Запускаем iperf c полосой пропускания 60 Mbps и через blue_red_switch_time сек смотрим как перестроился RSVP LSP
    result_loss=0
    packet_send=0
    packet_recv=0
...
# Проверим что есть пинг до iperf-сервера, иначе смысла нет ждать когда законфится тест
    time.sleep(10) # Подождем пока docker-контейнеры стартанут....
...
# Для этого ждем пока время текущее время станет больше чем stop_traffic_gen + red_blue_switch_time сек 

    
    current_time=time.time()
    conn.execute('show mpls rsvp lsp autobandwidth')
    resp_ab=conn.response
    template_ab = open('./templates/parse_show_mpls_rsvp_lsp_autobandwidth.txt')
    fsm_ab = textfsm.TextFSM(template_ab)
    processed_result_ab = fsm_ab.ParseTextToDicts(resp_ab)
    max_bw_interval=int(processed_result_ab[0]['sample_int_max_bw']) # Получаем значение максимального sample измерения трафика на текщем adjust интервале
    last_bw=int(processed_result_ab[0]['last_bw']) # Получаем значение последнего sample измерения
    while current_time <= (stop_traffic_gen+red_blue_switch_time):
    	conn.execute('show mpls rsvp lsp autobandwidth')
    	resp_ab=conn.response
    	template_ab = open('./templates/parse_show_mpls_rsvp_lsp_autobandwidth.txt')
    	fsm_ab = textfsm.TextFSM(template_ab)
    	processed_result_ab = fsm_ab.ParseTextToDicts(resp_ab)
#    	print(processed_result_ab) # Раскомментируй если хочешь контролировать как меняются параметры AB
    	max_bw_interval=int(processed_result_ab[0]['sample_int_max_bw']) # Получаем значение максимального sample измерения трафика на текщем adjust интервале
    	last_bw=int(processed_result_ab[0]['last_bw']) # Получаем значение последнего sample измерения
    	underflow_limit=int(processed_result_ab[0]['underflow_limit'])
    	underflow_count=int(processed_result_ab[0]['underflow_count'])
    	print('\rШаг3. До проверки переключения с красного на синий путь осталось - %d\r'%(red_blue_switch_time-(current_time-stop_traffic_gen)))
    	time.sleep(15)
    	current_time=time.time()
    	if (max_bw_interval < 6000 and last_bw == 0 and (underflow_count+1==underflow_limit)):
    		time.sleep(60) # ожидаем один sample интервал и выходим из цикла т.к. это условие срабатывания underflow триггера
    		print('\rСработал underflow триггер, выходим из цикла ожидания  Шага 3. Прошедшее время с окончания генерации трафика - %d сек.\r'%(current_time+60-stop_traffic_gen))
    		break

    conn.execute(cmd)

    check_time_step3=time.time()  # Фиксируем время проверки на шаге 3: C Красного на Синий путь
    check_time_step3_struct=time.localtime(check_time_step3)

    print('Шаг3. %s Проверяем как перестроился туннель %s\r'%(time.strftime('%H:%M:%S',check_time_step3_struct),tun_name))
    resp3=conn.response
    allure.attach(resp3, 'Шаг3.Вывод команды %s после отсутствия трафика в течение %d секунд'%(cmd,red_blue_switch_time), attachment_type=allure.attachment_type.TEXT)    
#    print(resp3)    
    template = open('./templates/parse_show_mpls_rsvp_lsps_tunnel_name.txt') 
