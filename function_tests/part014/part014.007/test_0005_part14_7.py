...
def restore_config_after_test5_part14_7(ip, login, password):
    yield
    conn = Telnet()
...
@allure.title('Проверка свичинга ICMP пакетов при срабатывании TE FRR Facility protection NHOP на atAR1')
...
@pytest.mark.parametrize('hostname,ip, login, password, int1', [(DUT1['hostname'],DUT1['host_ip'], DUT1['login'], DUT1['password'], 'bu2')])
@pytest.mark.usefixtures('restore_config_after_test5_part14_7')
def test_switching_L3VPN_TE_FRR_Faclity_NHOP_protection_atAR1_part14_7(hostname, ip, login, password, int1):
    ...
    allure.attach.file('./network-schemes/part14_7_switching_L3VPN_TE_FRR_Facility_NHOP_protection_atAR1.png',
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        time.sleep(5)  # Подождем пока трафик генерируется в параллельном потоке через защищаемый RSVP LSP
    ...
    assert_that(lsp_signal_int == 'Bundle-ether1',
                "Шаг2. В выводе команды %s параметр Signalling interface не равен ожидаемому значению Bundle-ether1, а равен - %s" % (
                cmd, lsp_signal_int))
    assert_that(plr_id == '1.0.0.3',
                "Шаг2. В выводе команды %s параметр PLR (LSP repaired at) не равен ожидаемому значению 1.0.0.3, а равен - %s" % (
                cmd, plr_id))
    ...
    # Шаг3. Проверяем работоспособность после возвращения на исходный путь
