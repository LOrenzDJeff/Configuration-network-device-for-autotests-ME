...
def restore_config_after_test4_part14_9(ip,login, password): # Передаём в фикстуру параметры: ip - адрес куда подключаемся; hostname -имя узла куда подключаемся
    yield
    conn = Telnet()
...
@allure.title('Проверка свичинга ICMP пакетов при срабатывании TE FRR Facility protection NNHOP на atDR1:')
...
@pytest.mark.parametrize('ip , login, password, int1', [(DUT3['host_ip'] , DUT3['login'] , DUT3['password'] , 'bu1')])
@pytest.mark.usefixtures('restore_config_after_test4_part14_9')
def test_switching_L3VPN_TE_FRR_Faclity_NNHOP_protection_atDR1_part14_9 (ip, login, password, int1):   
...
# перенаправлен в защитный RSVP LSP bypass  
    ...
    allure.attach.file('./network-schemes/part14_9_L3VPN_TE_FRR_Facility_NNHOP_protection_atDR1.png','Схема Теста', attachment_type=allure.attachment_type.PNG)
    conn = Telnet()
    acc = Account(login , password)
...
#    print(processed_result)
    ...
    assert_that(lsp_signal_int=='Tunnel-rsvpbypass@bypass',"Шаг2. В выводе команды %s параметр Signalling interface не равен ожидаемому значению Tunnel-rsvpbypass@bypass, а равен - %s"%(cmd,lsp_signal_int))
    assert_that(plr_id=='1.0.0.1',"Шаг2. В выводе команды %s параметр PLR (LSP repaired at) не равен ожидаемому значению 1.0.0.1, а равен - %s"%(cmd,plr_id))
...
# Запустим генерацию трафика с ESR-a путем запуска функции generate_uu_trafic_from_labr02 данный способ позволяет получить результат из функции в родительский thread
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        conn.execute('end')
        print('Шаг3. Выполнили no shutdown интерфейса %s на %s\r'%(int1,ip))
        param_list = future.result() # Получаем из параллельного потока значение % потерянных пакетов и кол-во отправленных и принятых пакетов!
        packet_loss = param_list[0]
        packet_sent = param_list[1]
        packet_recv = param_list[2]
