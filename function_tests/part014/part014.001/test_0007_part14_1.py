...
@allure.title('Проверка вывода команды show mpls rsvp tunnels-lsp interface')
@pytest.mark.part14_1
@pytest.mark.show_rsvp_lsp_interface
@pytest.mark.parametrize('ip, login, password, int1', [(DUT3['host_ip'], DUT3['login'], DUT3['password'], 'bu1'),(DUT2['host_ip'], DUT2['login'], DUT2['password'], 'bu2'),(DUT1['host_ip'], DUT1['login'], DUT1['password'], 'bu1')])
def test_show_rsvp_tunnel_lsp_int_part14_1 (ip, login, password, int1):
    allure.attach.file('./network-schemes/part14_1_show_mpls_rsvp_tunnel-lsp-interface.png','Схема Теста', attachment_type=allure.attachment_type.PNG)     
    resp = ''
    conn = Telnet()
    ...
    if ip == DUT3['host_ip']:
        ...
        allure.attach.file('./network-schemes/part14_1_show_mpls_rsvp_tunnel-lsp-interface-atDR1.png','Вывод команды', attachment_type=allure.attachment_type.PNG) 
    elif ip == DUT2['host_ip']:
        conn.set_prompt('atAR2#')
        allure.attach.file('./network-schemes/part14_1_show_mpls_rsvp_tunnel-lsp-interface-atAR2.png','Вывод команды', attachment_type=allure.attachment_type.PNG) 
    elif ip == DUT1['host_ip']:
        conn.set_prompt('atAR1#')
        allure.attach.file('./network-schemes/part14_1_show_mpls_rsvp_tunnel-lsp-interface-atAR1.png','Вывод команды', attachment_type=allure.attachment_type.PNG) 
    conn.execute('terminal datadump')
    cmd=('show mpls rsvp lsps interface %s'%int1)
    conn.execute(cmd)
    resp=conn.response
...
#    resp_output=resp.partition(cmd) # Данное действие необходимо чтобы избавиться от 'мусора' в выводе
    ...
    template = open('./templates/parse_show_mpls_rsvp_tunnel-lsp-interface.txt') 
...
# Попробуем найти LSP по имени в словаре который сформировался после парсинга    
    ...
    lsp1_int_in=processed_result[located_index1]['lsp_int_in']
    lsp1_int_out=processed_result[located_index1]['lsp_int_out']
    ...
    lsp2_int_in=processed_result[located_index2]['lsp_int_in']
    lsp2_int_out=processed_result[located_index2]['lsp_int_out']

    assert_that(Top!='',"Заголовок в выводе команды show mpls rsvp tunnels-lsp interface %s не соответствует шаблону"%Top)
    if ip==DUT3['host_ip']:
        ...
        assert_that(lsp1_int_in!='',"Значение входящего интерфейса  для RSVP LSP1 %s не соответствует шаблону"%lsp1_int_in)
        assert_that(lsp1_int_out!='',"Значение исходящего интерфейса  для RSVP LSP1 %s не соответствует шаблону"%lsp1_int_out)
        ...
        assert_that(lsp2_int_in!='',"Значение входящего интерфейса  для RSVP LSP2 %s не соответствует шаблону"%lsp2_int_in)
        assert_that(lsp2_int_out!='',"Значение исходящего интерфейса  для RSVP LSP2 %s не соответствует шаблону"%lsp2_int_out)
