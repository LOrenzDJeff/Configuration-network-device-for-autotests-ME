...
def test_banner_login_tacacs(DUT):
    ...
    resp= connection_test(router.host_ip,'admin','test_password','part2')
    allure.attach(resp,'Вывод banner login и banner motd для %s'%router.host_ip, attachment_type=allure.attachment_type.TEXT)
    assert_that(resp.find(banner_login)!=-1,"Ожидаемое сообщение banner_login - %s не обнаружено при подключении к узлу %s"%(banner_login, router.host_ip))
    assert_that(resp.find(banner_login_1)!=-1,"Ожидаемое сообщение banner_login_1 - %s не обнаружено при подключении к узлу %s, хотя tacacs-сервер включен"%(banner_login_1, router.host_ip))
    assert_that(resp.find(banner_motd)!=-1,"Ожидаемое сообщение banner_motd - %s не обнаружено при подключении к узлу %s"%(banner_motd, router.host_ip))
