...
@allure.title('Проверка banner motd и banner login для telnet-соединения при выключенном tacacs-сервере')
...
def test_banner_login(host_ip , login , password):
    ...
    res1=conn.response
    res=res +res1
    allure.attach(res,'Вывод banner login и banner motd для %s'%host_ip, attachment_type=allure.attachment_type.TEXT)
    assert_that(res.find(banner_login)!=-1,"Ожидаемое сообщение banner_login - %s не обнаружено при подключении к узлу %s"%(banner_login,host_ip))
    assert_that(res.find(banner_motd)!=-1,"Ожидаемое сообщение banner_motd - %s не обнаружено при подключении к узлу %s"%(banner_motd,host_ip))
